...
def view_product_by_uuid(request, product_uuid):
    """Look up product by UUID and redirect to slug-based URL."""
    try:
        product = Product.objects.get(id=product_uuid)
        return HttpResponseRedirect(reverse('products:view_product', kwargs={'slug': product.slug}))
...
def show_products_category_by_uuid(request, category_uuid):
    """Look up category by UUID and redirect to slug-based URL."""
    try:
        category = Category.objects.get(id=category_uuid)
        return HttpResponseRedirect(reverse('products:show_products_category', kwargs={'slug': category.slug}))
    except (ObjectDoesNotExist, ValueError):
        raise Http404("Category not found")

def show_products_category(request, slug):
