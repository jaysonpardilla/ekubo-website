...
def seller_dashboard(request):
    ...
    # Get categories used by seller's products
    categories_used = Category.objects.filter(products__seller__user=request.user).distinct()
    
    # If no categories with products, show all categories from database
    if categories_used.exists():
        total_categories = categories_used.count()
    else:
...
def view_category(request):
    # Show all categories that the seller's products use, or all categories if none have products yet
    # First check if there are any categories with products
    categories_with_products = Category.objects.filter(products__seller__user=request.user).distinct()
